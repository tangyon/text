	...
	<div class="lately">
		<div class="carousel">
			...
			<van-swipe :autoplay="3000" indicator-color="white">
			  <van-swipe-item class="carousel-box" 
			  	v-for="(item,index) in hotData " :key="index">
			  	<div class="img-box" 
			  		:style="{background:`url(${item.coverImgUrl}) 50% / 100% auto `}"
		...
		<van-row class="lately-box">
			<h4>精选歌单</h4>
		  	<van-col span="11"  v-for="(item,index) in BoutiqueData" :key="index" 
		  		:offset="index%2 ==0 ? 0 :2">	
		  		 
		  		 	<div class="img" :style="{background:`url(${item.coverImgUrl}) 50% / auto 100%`}"
		  		 		@click="godetails(item)">
		  		 			<p>{{item.tag}}</p>
