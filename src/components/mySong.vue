<template>
	...
	<div class="album">
		<div class="top">
			<van-nav-bar :title="data.songListName" left-arrow @click-left="onClickLeft" ref="nav" />
		</div>

		<div class="description">
			<div class="explain">
				<div class="float">
					<img class="explain-img" :src=data.data[0].pic />
				</div>
			...
			<div class="number-box">
				...
				<ul class="album-box-list">
					...
					<li class="album-list" v-for="(item ,index) in data.data" :key="index" @click="goplay(index)">

						<van-row>
							<van-col span="2">
								<span ref="iconIndex" :class="index==defaultIndex?'glyphicon glyphicon-volume-up':'index'" @click.stop="toplay(index)">
							{{index==0?'':index+1}}
						</span>
							</van-col>
							...
							<van-col offset='1' span="2">
								<span class="glyphicon glyphicon-list menu" @click.stop="menu(index)"></span>
							</van-col>
						</van-row>
					</li>
				</ul>
		...
		<van-actionsheet v-model="show">
			<div class="show-title">
				<span class="show-title-img" :style="{background:`url(${presentMusic.pic})50% / auto 100%`}">
		...
		methods: {
			...
			toplay(index) {
				...
				if(icon[index].getAttribute('class') == 'glyphicon glyphicon-volume-up') {
					if(this.data.data[0].id != this.datas[0].id) {
						this.$store.commit("setKeywordData", this.data.data)
					}
					this.$store.commit("setIndex", index)
			...
			deleteMusic() {
				...
				this.data.data.splice((this.indexI), 1)
				if(this.indexI==0){
					this.indexI=0
				}else{
					this.indexI--
